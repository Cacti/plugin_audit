...
/*
 * Unit/integration coverage for Syslog record formatting (RFC 5424/CEF) and
 * transport delivery (UDP/TCP/TLS) in audit_syslog.php. Socket- and
 * OpenSSL-dependent sections are skipped defensively when the relevant
 * extensions are unavailable, matching the original standalone script.
 */
...
/**
 * Pest loads every spec file into one process, so assertion failures here
 * must not call exit()/die() the way the original standalone script did -
 * that would abort the entire test run instead of just this spec.
 */
function audit_syslog_test_assert($condition, $message) {
	expect((bool) $condition)->toBeTrue($message);
}

function audit_syslog_test_config($overrides = array()) {
	$values = array(
		'receiver'            => '127.0.0.1',
		'port'                => '514',
		'transport'           => 'udp',
...
it('formats RFC 5424/CEF Syslog records and delivers them over UDP, TCP, and TLS', function () {
